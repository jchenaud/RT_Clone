...
/*   Updated: 2017/08/24 01:05:09 by pribault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_img	*import_bmp(char *file)
{
	...
	read(fd, &dib, sizeof(t_bmp_dibh)) != sizeof(t_bmp_dibh))
		return (NULL);
	new->img = (t_color*)malloc(sizeof(t_color) * dib.width * dib.height);
	ft_bzero(new->img, sizeof(t_color) * dib.width * dib.height);
	new->w = dib.width;
	new->h = dib.height;
